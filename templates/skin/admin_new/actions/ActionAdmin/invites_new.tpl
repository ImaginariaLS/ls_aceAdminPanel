...
{if !$aNewInviteList}
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <p>
            {$oLang->settings_invite_available}:
            <strong>{if $iCountInviteAvailable==-1}{$aLang.settings_invite_many}{else}{$iCountInviteAvailable}{/if}</strong><br/>
            {$oLang->settings_invite_used}: <strong>{$iCountInviteUsed}</strong>
        </p>
        <p>
            <label class="radio">
                <input type="radio" name="adm_invite_mode" id="adm_invite_mode_mail" value="mail"
                       {if $sInviteMode=='mail'}checked{/if} onclick="aceAdmin.inviteMode('mail');"/>
                {$oLang->adm_invite_mode_mail}</label>

            <label class="radio">
                <input type="radio" name="adm_invite_mode" id="adm_invite_mode_text" value="text"
                       {if $sInviteMode=='text'}checked{/if} onclick="aceAdmin.inviteMode('text');"/>
        ...
        <div id="div_invite_text" {if $sInviteMode=='mail'}style="display:none;"{/if}>
            ...
            <input type="text" name="invite_count" id="invite_count" class="w100" style="text-align:right;"
                   value="{$iInviteCount}"/><br/>
        </div>
        <br/>
